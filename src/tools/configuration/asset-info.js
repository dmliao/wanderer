...
const getPageInfo = async (baseDir, targetFile, globalConfig) => {
	...
	const specialFilenames = ['index', '404']

	return {
		...assetInfo,
		// unlisted ones don't get next and previous pages, and don't show up in next / previous lists
		isUnlisted: specialFilenames.indexOf(assetInfo.pageName) >= 0,
		// pages with no pretty URL don't get their own folder. This is mostly to avoid creating redundant index folders.
		hasPrettyURL: specialFilenames.indexOf(assetInfo.pageName) < 0,
		isPage: true,
		text: pageText,
		title,
	}
