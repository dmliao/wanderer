...
const harpe = (options) => {
    ...
    */
	const inlineTypes = options.inlineTypes || {
		strong: {
			// the first half matches all text wrapped in code blocks so we can ignore it.
			match: /`[^`]+`|(\*\*(\S(.*?\S)?)\*\*)/gm,
			out: (match, p0, p1, p2) => {
				// match is the full matching text
				// p0-p2 are the capture groups

				// if we match the code brackets, we should automatically terminate
				// matching code brackets will terminate before we match any groups
				if (!p1) {
					return match
				}
			...
			_p: 10
		}, // only matches ** version
		em: {
			match: /`[^`]+`|(_(\S(.*?\S)?)_(?![A-z]))/gm,
			out: (match, p0, p1, p2) => {
				// match is the full matching text
				// p0-p2 are the capture groups
	...
	const defineBlockType = (match, newType, priority) => {
		...
		if (!valid) {
			console.warn(
				"Adding invalid block type. You need either a plain string 'tag' or HTML 'open' and 'close' properties"
			)
		}
	...
	const defineInlineType = (name, newType, priority) => {
		...
		if (!newType.match || !newType.out) {
			console.warn(
				"Adding invalid inline type. The type needs a 'match', which is a regex to match the syntax, and an 'out', which uses capturing groups to replace the string with HTML"
			)
			return
		}
		...
		for (let line of lines) {
			...
			switch (type) {
				...
				case 'table':
					...
					finalString +=
						processTable(textWithoutRune, isNewTable) + '\n' // TODO: implement this!
					continue
				case 'hr':
					if (line.trim() === '---') {
						finalString += '<hr />\n'
			...
			for (let b in blockTypes) {
				...
				finalString +=
					wrap(
						textWithoutRune,
						tagDefinition.open || `<${tagDefinition.tag}>`,
						tagDefinition.close || `</${tagDefinition.tag}>`
					) + '\n'
				processedBlock = true
				break
			}
